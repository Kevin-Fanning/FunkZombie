...
struct SpriteInfo
{
	...
	SpriteInfo(int _texID, int _x, int _y, int _w, int _h, int _imgX, int _imgY, int _sx, int _sy, int _sw, int _sh, float _r, float _g, float _b)
		: texID(_texID), x(_x), y(_y), w(_w), h(_h), imgX(_imgX), imgY(_imgY), sx(_sx), sy(_sy), sw(_sw), sh(_sh), r(_r), g(_g), b(_b) {}
